...
class PayloadRepository(private val context: Context) {
    fun loadTargets(): List<TargetProfile> {
        ...
        ) }.let(::patchSamsungS24UltraUS)
    }

    /**
     * Adds SM-S928U (US carrier Galaxy S24 Ultra) to profiles that already
     * support SM-S928U1 (US unlocked variant). Both variants share identical
     * hardware (Snapdragon 8 Gen 3) and kernel, so the exploit payload is
     * fully compatible.
     */
    private fun patchSamsungS24UltraUS(profiles: List<TargetProfile>): List<TargetProfile> =
        profiles.map { profile ->
            if ("SM-S928U1" in profile.models && "SM-S928U" !in profile.models) {
                profile.copy(models = profile.models + "SM-S928U")
            } else {
                profile
            }
        }
